#include "AES_encrypt.hpp"
#include <cstring>

const std::array<uint8_t, 256> AES::sbox = {
    
};


const std::array<uint8_t, 256> AES::invSbox = {
    
};

// Round constant (Rcon) array specific to AES-128 (only the first 10 are used)
const std::array<uint8_t, 11> AES::rcon = {
    0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B
};


AES::AES(const std::vector<uint8_t>& key) {
    if (key.size() == 16) {
        Nk = 4;
        Nr = 10;
    } else if (key.size() == 24) {
        Nk = 6;
        Nr = 12;
    } else if (key.size() == 32) {
        Nk = 8;
        Nr = 14;
    } else {
        throw std::invalid_argument("Invalid key size.");
    }
    keyExpansion(key);
}

void AES::keyExpansion(const std::vector<uint8_t>& key) {
    const size_t keyWords = key.size() / 4;  // keyWords for AES-128 is always 4
    const size_t expandedKeySize = Nb * (Nr + 1);  // Expanded key size in words

    // First copy the original key to the expanded key
    for (size_t i = 0; i < keyWords; ++i) {
        expandedKey[4 * i + 0] = key[4 * i + 0];
        expandedKey[4 * i + 1] = key[4 * i + 1];
        expandedKey[4 * i + 2] = key[4 * i + 2];
        expandedKey[4 * i + 3] = key[4 * i + 3];
    }

    // Variables to hold temporary words
    uint32_t temp;
    for (size_t i = keyWords; i < expandedKeySize; ++i) {
        temp = (expandedKey[4 * (i - 1) + 0] << 24) |
               (expandedKey[4 * (i - 1) + 1] << 16) |
               (expandedKey[4 * (i - 1) + 2] << 8) |
               (expandedKey[4 * (i - 1) + 3]);

        if (i % keyWords == 0) {
            // Rotate the 4-byte word and apply S-box substitution
            temp = (temp << 8) | (temp >> 24);  // Rotate left by 8 bits
            temp = (sbox[(temp >> 24) & 0xFF] << 24) |
                   (sbox[(temp >> 16) & 0xFF] << 16) |
                   (sbox[(temp >> 8) & 0xFF] << 8) |
                   (sbox[temp & 0xFF]);

            // XOR with round constant
            temp ^= (uint32_t(rcon[i / keyWords]) << 24);
        }

        // XOR with the word Nk positions before
        temp ^= (expandedKey[4 * (i - keyWords) + 0] << 24) |
                (expandedKey[4 * (i - keyWords) + 1] << 16) |
                (expandedKey[4 * (i - keyWords) + 2] << 8) |
                (expandedKey[4 * (i - keyWords) + 3]);

        expandedKey[4 * i + 0] = temp >> 24;
        expandedKey[4 * i + 1] = (temp >> 16) & 0xFF;
        expandedKey[4 * i + 2] = (temp >> 8) & 0xFF;
        expandedKey[4 * i + 3] = temp & 0xFF;
    }
}

void AES::addRoundKey(std::array<uint8_t, 16>& state, int round) {
    size_t start = round * Nb * 4;
    for (int i = 0; i < 16; ++i) {
        state[i] ^= expandedKey[start + i];
    }
}

void AES::subBytes(std::array<uint8_t, 16>& state) {
    for (int i = 0; i < 16; ++i) {
        state[i] = sbox[state[i]];
    }
}


void AES::shiftRows(std::array<uint8_t, 16>& state) {
    std::array<uint8_t, 16> temp = state;
    // Row 0 doesn't shift

    // Row 1 shifts left by 1
    state[1] = temp[5];
    state[5] = temp[9];
    state[9] = temp[13];
    state[13] = temp[1];

    // Row 2 shifts left by 2
    state[2] = temp[10];
    state[6] = temp[14];
    state[10] = temp[2];
    state[14] = temp[6];

    // Row 3 shifts left by 3
    state[3] = temp[15];
    state[7] = temp[3];
    state[11] = temp[7];
    state[15] = temp[11];
}


void AES::mixColumns(std::array<uint8_t, 16>& state) {
    std::array<uint8_t, 16> temp = state;
    for (int i = 0; i < 4; i++) {
        state[i * 4 + 0] = gmul(temp[i * 4 + 0], 0x02) ^ gmul(temp[i * 4 + 1], 0x03) ^ temp[i * 4 + 2] ^ temp[i * 4 + 3];
        state[i * 4 + 1] = temp[i * 4 + 0] ^ gmul(temp[i * 4 + 1], 0x02) ^ gmul(temp[i * 4 + 2], 0x03) ^ temp[i * 4 + 3];
        state[i * 4 + 2] = temp[i * 4 + 0] ^ temp[i * 4 + 1] ^ gmul(temp[i * 4 + 2], 0x02) ^ gmul(temp[i * 4 + 3], 0x03);
        state[i * 4 + 3] = gmul(temp[i * 4 + 0], 0x03) ^ temp[i * 4 + 1] ^ temp[i * 4 + 2] ^ gmul(temp[i * 4 + 3], 0x02);
    }
}

// Helper function to perform multiplication in GF(2^8)
uint8_t AES::gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) {
            p ^= a;
        }
        bool high_bit_set = (a & 0x80);
        a <<= 1;
        if (high_bit_set) {
            a ^= 0x1b; // x^8 + x^4 + x^3 + x + 1
        }
        b >>= 1;
    }
    return p;
}

std::vector<uint8_t> AES::EncryptECB(const std::vector<uint8_t>& data) {
    // Assuming data size is a multiple of 16 bytes (128 bits), if not, it should be padded accordingly
    size_t dataSize = data.size();
    std::vector<uint8_t> encryptedData(dataSize);  // Create a vector for the encrypted data

    std::array<uint8_t, 16> block;  // Temporary block to store data during encryption

    for (size_t i = 0; i < dataSize; i += 16) {
        // Copy the current block to encrypt
        std::copy(data.begin() + i, data.begin() + i + 16, block.begin());

        // Perform encryption on the block
        addRoundKey(block, 0);  // Initial round key addition

        for (int round = 1; round < Nr; ++round) {
            subBytes(block);
            shiftRows(block);
            mixColumns(block);
            addRoundKey(block, round);
        }

        // Final round (does not include mixColumns)
        subBytes(block);
        shiftRows(block);
        addRoundKey(block, Nr);

        // Copy the encrypted block back to the output data
        std::copy(block.begin(), block.end(), encryptedData.begin() + i);
    }

    return encryptedData;
}



std::vector<uint8_t> AES::decrypt(const std::vector<uint8_t>& ciphertext) {
    // Implement decryption here
    return {}; // placeholder
}

